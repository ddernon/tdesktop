...
QByteArray Settings::serialize() const {
	...
	size += sizeof(qint32) * 2
		...
		+ sizeof(qint32) * 8
		+ sizeof(ushort)
		+ Serialize::stringSize(_notificationsDisplayId);
...
void Settings::addFromSerialized(const QByteArray &serialized) {
	...
	QString notificationsDisplayId = _notificationsDisplayId;
	...
	_notificationsDisplayId = std::move(notificationsDisplayId);
